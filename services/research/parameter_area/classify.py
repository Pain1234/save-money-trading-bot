...
def classify_parameter_area(
    *,
    ...
    # Isolated peak before steep-drop UNSTABLE: a sharp drop to unstable
    # neighbors is exactly the isolated-optimum signature.
    if frozen_stable and not any_stable_neighbor:
        return ("ISOLATED_PEAK", "frozen_stable_without_stable_neighbors")

    if not frozen_stable and not any_stable_neighbor:
    ...
    if plateau_size < policy.narrow_min_contiguous:
        if frozen_stable:
            return (
                "ISOLATED_PEAK",
    ...
    if plateau_size >= policy.narrow_min_contiguous and stable_share >= narrow_share:
        # Missing gates blocks BROAD but still allows NARROW when cost+PnL stable.
        return (
            "NARROW_STABLE_AREA",
            f"plateau_size={plateau_size}, stable_share={stable_share}, "
